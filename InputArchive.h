...
namespace Archives
{
	...
	private:
		...
		constexpr ArchiveType& self() noexcept
		{
		...
		{
			static_assert(std::is_lvalue_reference<T>, "Passed rvalue reference for loading to Input Archive! \n (Impossible since load can not write a value into a temporary)");
		...
		//TODO: use same trick as dowork to implement special behavior for beforework and afterwork
		template <typename T>
		inline void beforework(T&& value)
		{ /*Use indirect overload in ArchiveType*/
		}
		...
		//TODO: use same trick as dowork to implement special behavior for beforework and afterwork
		template <typename T>
		inline void afterwork(T &&value)
		{  /*Use indirect overload in ArchiveType*/	}

	protected:
		constexpr InputArchive(const ArchiveType*) noexcept {};
